...
def main():
    ...
    if (len(sys.argv) == 1) : 
        print("Usage ListNaiz.py  [Option:0~2]")
        print("      ListNaiz.py  0  <== Check New Camera & Name Change")
        print("      ListNaiz.py  1  <== Check New Camera & Name Change & Check Delete Camera")
        print("      ListNaiz.py  2  <== Only VMS_IP,VMS_CH field update")
        return

    naiz_url = Config_get_NaizList("getlist")
...
# 프로그램 수행후 값이 0 이면 삭제된 것으로 판단할수 있다.    
    ...
    if (ChkDB == 2) : 
        ...
        for row in rows :
            ...
            vms_ch = rtsp1.replace('rtsp://',"") 
            vms_ch = vms_ch[vms_ch.find("/")+1:]
            vms_ch = vms_ch.replace('stream1',"") 
            vms_ch = vms_ch.replace('/',"") 
            sql_stmt = "update cameralist set vms_ip='"+vms_ip+"',vms_ch='"+vms_ch + "' where seq = "+str(row[0])+";"
...
# 해당키로 기존에 있는 리스트라면 CheckUpdate 값을 1 로 변경                
                    ...
                    if (findname != Name) or (ipaddr != IP_Addr) or (rtsp1 != RTSP_URL1) :
                        ...
                        sql_stmt = "update cameralist set device_name='"+Name+"',ip_addr='"+IP_Addr+"',rtsp_url1='"+RTSP_URL1+"',rtsp_url2='"+RTSP_URL2
                        sql_stmt = sql_stmt + "',vms_ip='"+vms_ip+"',vms_ch='"+vms_ch + "' where seq = "+UniqueKey+";"
                        cur.execute( sql_stmt )
                        Naiz_DB.commit()
